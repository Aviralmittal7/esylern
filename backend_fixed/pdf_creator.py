...
class WorksheetPDF(FPDF):
    ...
    def header_block(self, subject, child_name, topic, generated_on):
        ...
        self.cell(60, 7, "Score: _______ / 20", border=0)
        self.ln(12)
        self.set_text_color(0, 0, 0)
    ...
    def body_text(self, text, size=12):
        ...
        for raw_line in text.split("\n"):
            ...
            # ("1.", "2)", "10.", "20)", etc.) without trying to be a full
            # markdown parser.  The original check used line[0].isdigit()
            # and line[1] in ".)" which only worked for activities 1-9;
            # activities 10-20 were not highlighted at all.
            PILL_COLOURS = [
                (255, 230, 100),   # yellow  – cycle position 1
                (200, 240, 200),   # green   – cycle position 2
                (200, 220, 255),   # blue    – cycle position 3
                (255, 215, 200),   # peach   – cycle position 4
            ]
            _num_match = re.match(r'^(\d{1,2})[.)]\s', line)
            if _num_match:
                self.ln(3)
                q_idx = int(_num_match.group(1)) - 1  # 0-based
