...
from email_sender import send_combined_worksheet_email, send_admin_alert
...
def process_parent(parent_id):
    ...
    logger.info("Generating %s worksheet(s) for %s (%s, subjects=%s)",
                len(subjects), parent["child_name"], parent["email"], subjects)
    ...
    try:
        for subj in subjects:
            ...
            worksheet_bytes = create_worksheet_pdf(student_text, parent["child_name"], subj, subj)
            answer_bytes = create_answer_pdf(answer_key, parent["child_name"], subj, subj)
            items.append({
                "subject": subj, "topic": subj,
                "worksheet_bytes": worksheet_bytes, "answer_bytes": answer_bytes,
                "model_used": model_used,
            })

        send_combined_worksheet_email(
        ...
        for i in items:
            db.record_delivery_success(
                parent_id,
                worksheet_file=f"{i['subject']}_{i['topic']}_worksheet.pdf",
                topic=i["topic"],
                difficulty=difficulty,
                summary=f"Sent via model {i['model_used']}",
            )
        logger.info("-> Sent %s worksheet(s) to %s", len(items), parent["email"])

    except Exception as e:  # noqa: BLE001 - top-level safety net for a background job
        logger.exception("Failed to process worksheets for parent_id=%s (%s): %s",
                         parent_id, parent["email"], e)
        failures = db.record_delivery_failure(parent_id, ",".join(subjects), difficulty, e)
