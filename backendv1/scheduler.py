...
def process_parent(parent_id):
    ...
    topic = (prefs["topic"] if prefs and prefs["topic"] else parent["subject_focus"])
    ...
    logger.info("Generating %s worksheet for %s (%s, topic=%s)",
                difficulty, parent["child_name"], parent["email"], topic)

    try:
        student_text, answer_key, model_used = generate_worksheet_text(
            grade=parent["grade_level"],
            subject=parent["subject_focus"],
            topic=topic,
        ...
        worksheet_bytes = create_worksheet_pdf(
            student_text,
            parent["child_name"], parent["subject_focus"], topic,
        )
        ...
        db.record_delivery_success(
            parent_id,
            worksheet_file=f"{parent['subject_focus']}_{topic}_worksheet.pdf",
            topic=topic,
            difficulty=difficulty,
